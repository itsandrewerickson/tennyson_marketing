# Property Data: 3754-56 Tennyson Street

> **This file is the single source of truth for all property information.**
> All marketing materials should pull from this document.

---

## Basic Information

| Field | Value |
|-------|-------|
| **Address** | 3754-56 Tennyson Street |
| **City** | San Diego |
| **State** | California |
| **Zip Code** | 92107 |
| **Neighborhood** | Point Loma |
| **APN** | 449-461-07-00 |
| **Legal Description** | BLK 20*LOT 8 |

---

## Pricing

| Field | Value |
|-------|-------|
| **Asking Price** | $1,790,000 |
| **Price/SF (Land)** | $320 |
| **Price/SF (Living)** | $1,332 |

---

## Land & Zoning

| Field | Value |
|-------|-------|
| **Lot Size (SF)** | 5,595 |
| **Usable SF** | 5,600 |
| **Zoning** | RM-3-7 Multifamily |
| **Property Type** | Multi-family |
| **Street Type** | Cul-de-sac |

---

## Existing Improvements

### Main House
| Field | Value |
|-------|-------|
| **Year Built** | 1941 |
| **Bedrooms** | 2 |
| **Bathrooms** | 1 |
| **Living SF** | 1,344 |
| **Condition** | Fully Renovated |
| **Features** | Full kitchen, living room, direct courtyard access |
| **Current Use** | Airbnb rental (as part of compound) |
| **STR License** | STR-12124L |
| **Registration ID** | 659128 |

### Existing ADU (Teardown)
| Field | Value |
|-------|-------|
| **Status** | Teardown scheduled |
| **Condition** | Poor - to be demolished |
| **Location** | Backyard |
| **Current Config** | 1-bed/1-bath suite with kitchenette |
| **Features** | Spa-style shower, boho-chic decor |
| **Current Use** | Rented with main house as Airbnb compound |

---

## Development Potential

### Approved ADU Plans
| Field | Value |
|-------|-------|
| **Total New ADUs** | 8 |
| **Unit Size (each)** | 470 SF |
| **Unit Type** | 1 Bedroom |
| **Ceiling Type** | Vaulted |
| **Market Rate Units** | 5 |
| **Deed Restricted Units** | 3 |
| **Total New SF** | 3,760 SF |
| **Plan Approval Date** | End of January 2025 |

### Post-Development Summary
| Field | Value |
|-------|-------|
| **Total Units** | 9 (1 main house + 8 ADUs) |
| **Total Living SF** | ~5,104 SF |
| **Parking** | Limited on-site parking available |

---

## The Grandfathered Advantage

### Regulatory Context
- **Effective Date**: Summer 2025
- **Change**: ADUs can no longer be built on cul-de-sac properties in San Diego
- **Impact**: These 8 approved ADUs are grandfathered under previous rules
- **Scarcity**: This development potential cannot be replicated on similar properties

### Key Selling Points
1. Plans locked in before regulatory change
2. 8-unit development on a cul-de-sac - now impossible to permit
3. Mix of market rate (5) and deed restricted (3) satisfies city requirements
4. Vaulted ceilings add premium feel to compact units

---

## Location Highlights

### Point Loma
- Coastal San Diego neighborhood
- Walking distance to beaches
- Strong rental demand
- High household incomes
- Limited new development (built-out area)

### Nearby Amenities
- **Liberty Station** - Food hall, Public Market, coffee shops, breweries (~5 min)
- **Sunset Cliffs** - Iconic sunset views (~10 min)
- **Ocean Beach** - Beach, pier, farmers market (~10 min)
- **Downtown San Diego** - Little Italy, Gaslamp, Petco Park (~10-12 min)
- **San Diego International Airport** (~10 min)
- **La Jolla** (~15 min)
- **Coronado** (~15 min)

### Drive Times
| Destination | Time |
|-------------|------|
| Ocean Beach | ~5 min |
| Downtown San Diego | ~15 min |
| San Diego Airport | ~10 min |
| La Jolla | ~20 min |

---

## Financial Data

### Current Income
| Field | Value |
|-------|-------|
| **Rental Type** | Short-term rental (Airbnb) |
| **Configuration** | 2-unit compound rented together |
| **STR License** | STR-12124L |
| **Notes** | Property has active STR permit; can generate income during development planning |

### Pro Forma (Post-Development)
[To be populated with detailed projections]

### Comparable Sales
[To be populated with recent sales data]

---

## Contact Information

### Seller Representatives
| Field | Value |
|-------|-------|
| **Company** | Next Wave Group |

#### Andrew Erickson
| Field | Value |
|-------|-------|
| **Phone** | (619) 322-3395 |
| **Email** | Andrew@TheNextWaveGroup.com |

#### Marcelo Valdez
| Field | Value |
|-------|-------|
| **Phone** | (858) 204-1337 |
| **Email** | Marcelo@TheNextWaveGroup.com |

---

## Assets Checklist

### Photography
- [ ] Main house exterior (front)
- [ ] Main house exterior (rear)
- [ ] Main house interior (kitchen)
- [ ] Main house interior (living room)
- [ ] Main house interior (bedrooms)
- [ ] Main house interior (bathrooms)
- [ ] Backyard / development area
- [ ] Existing ADU (teardown)
- [ ] Street view
- [ ] Aerial/drone shots
- [ ] Neighborhood context

### Plans & Renderings
- [ ] Site plan (existing)
- [ ] Site plan (proposed with 8 ADUs)
- [ ] ADU floor plans
- [ ] ADU renderings (exterior)
- [ ] ADU renderings (interior if available)

### Branding
- [ ] Next Wave Group logo (PNG, SVG)
- [ ] Brand colors (hex codes)
- [ ] Font specifications

---

## Disclaimers

*All information is believed to be accurate but should be independently verified. Financial projections are estimates only and do not guarantee future performance. Buyer responsible for confirming zoning, permits, and development feasibility. Plans subject to final city approval.*
